...
func createProfile(motifs []string) [][]float64 {
	...
	for i := 0; i < k; i++ {
		...
		total := len(motifs)
		profile[0][i] = float64(counts['A']) / float64(total)
		profile[1][i] = float64(counts['C']) / float64(total)
		profile[2][i] = float64(counts['G']) / float64(total)
		profile[3][i] = float64(counts['T']) / float64(total)
	}
...
func main() {
	file, err := os.Open("rosalind_ba2d.txt")
	if err != nil {
